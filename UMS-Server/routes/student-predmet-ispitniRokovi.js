...
module.exports = {
  method: 'GET',
  path: '/api/student/predmet/ispitniRokovi',
  config: {
    handler: (req, res) => {
      let con = mysql.createConnection(connection);
      
      let sql = 'select ir.id ispitniRokId, ir.rokZaPrijave, ir.vrijemeOdrzavanja, s.naziv sala, '+
				...
				'from korisnik ko, student st, nastava n, ispitnirok ir, sala s, tipispita ti '+
				'where ko.id=st.Korisnik_id and st.id=n.Student_id and n.Kurs_id=ir.Kurs_id and n.Kurs_id=? and '+
					  'ir.Sala_id=s.id and ir.tip=ti.id and ko.username=? '+
				'order by ir.rokZaPrijave desc';
      let query = con.query(sql, [req.query.username1, req.query.kursId, req.query.username2], function(err, result){
        if (!err){
          res(result);      
        }
      });    
